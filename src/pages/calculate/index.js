...
export default function Overview() {
	...
	const handleOnGoToAssessment = useCallback(
		(type, id) => () => {
			...
			const lastIndex = type === 'product' ? product?.length : company?.length;
			id = id === -1 ? lastIndex + 1 : id;
			dispatch(setCalculatorContext({ active: id, name: type, step: 0 }));
			if (id < 0) {
				dispatch(action({ name: 'New Assessment', description: 'New Assessment', year: dayjs().year(), id: id }));
			}
			router.push(`/calculate/${id}`);
		},
		[company?.length, product?.length]
	);
	return (
